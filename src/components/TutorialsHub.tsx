...
import { Calendar, ArrowRight, BookOpen, Layers, ShieldCheck, Sparkles } from "lucide-react";
...
export default function TutorialsHub({ locale = 'zh' }: TutorialsHubProps) {
  ...
  const allArticles = useMemo(() => {
    return Object.values(seoData).filter(item => !CORE_PILLAR_KEYS.has(item.route));
  }, [seoData]);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-zinc-900">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/25 text-xs text-yellow-500 font-bold font-mono">
              ...
              <span>{isHant ? "官方實操與排錯知識庫全集" : "官方实操与排错知识库全集"}</span>
            </div>
            ...
            <p className="text-zinc-400 text-sm max-w-2xl leading-relaxed">
              {isHant 
                ? `共收錄 ${allArticles.length} 篇深度原創實操教程，涵蓋多屏看盤、C2C安全出金、合約風控防爆倉、Web3多鏈防盜等全流程指引。`
                : `共收录 ${allArticles.length} 篇深度原创实操教程，涵盖多屏看盘、C2C安全出金、合约风控防爆仓、Web3多链防盗等全流程指引。`}
            </p>
          </div>

          <div className="flex items-center gap-3 bg-zinc-900/50 border border-zinc-800 p-3 rounded-2xl shrink-0">
            <div className="text-right">
              <span className="block text-[11px] text-zinc-500 uppercase font-mono font-bold">Total Guides</span>
              <span className="text-lg font-black text-yellow-500 font-mono">{allArticles.length} 篇精选</span>
            </div>
