...
const IconWeb = () => (
    <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#eab308" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const IconApple = () => (
    <svg viewBox="0 0 24 24" width="20" height="20" fill="#eab308" stroke="none">
...
export default function DownloadGateway({
    ...
    // Effect to load config when opened (only on user click to avoid automated crawler detection)
    useEffect(() => {
    ...
    const processUrl = useCallback((url: string) => {
        if (!url) return '';
        // 替换 join/CODE 格式
        let processed = url.replace(/join\/([A-Za-z0-9_-]+)/gi, `join/${localConfig.invitationCode}`);
        // 替换 channelId=CODE 或 invitationCode=CODE 格式
        processed = processed.replace(/(channelId|channelid|invitationCode)=([A-Za-z0-9_-]+)/gi, `$1=${localConfig.invitationCode}`);
        return processed;
    }, [localConfig.invitationCode]);
        ...
        <div className={styles.overlay} onClick={(e) => {
            ...
            <div className={styles.modal}>
                <div className={styles.header}>
                    <h3 className={styles.title}>查看访问入口</h3>
                    {isLoading && (
                ...
                {isLoading ? (
                    ...
                    <>
                        {/* Domestic Group (Green/Orange Theme) */}
                        <div className={styles.group}>
                            <h4 className={styles.groupTitle} style={{ color: '#f59e0b' }}>
                            ...
                            <div className={styles.groupButtons}>
                                <button className={`${styles.actionBtn} ${styles.btnRecommend}`} onClick={() => handleDownload(config.web.china, 'link')}>
                                    ...
                                    <span className={styles.btnMain}>查看访问入口</span>
                                ...
                                <button className={`${styles.actionBtn} ${styles.btnRecommend}`} onClick={() => handleDownload(config.android.china, 'download')}>
                                    ...
                                    <span className={styles.btnTag}>🔥</span>
                                    <span className={styles.btnSub}>原装APK安装包</span>
                                </button>
                                <button className={styles.actionBtn} onClick={() => handleDownload(config.web.china, 'link')}>
                                    ...
                                    <span className={styles.btnSub}>需先注册或登录</span>
                                </button>
                            </div>
                        </div>
