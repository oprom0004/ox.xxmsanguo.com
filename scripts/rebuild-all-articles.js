...
// Search Intent Suffix Pools (Simplified Chinese)
const scSuffixes = [
  " - 欧意okx下载",
  " - 欧易okx网址",
  " - 欧意okx网站",
...
// Search Intent Suffix Pools (Traditional Chinese)
const hantSuffixes = [
  " - 歐意okx下載",
  " - 歐易okx網址",
  " - 歐意okx網站",
...
// 120 Rich, High-Quality SEO Article Topics
const topics = [
  {
    slug: "okx-futures-margin-ratio",
    tabLabel: "合约维持保证金",
    topicBody: "永续合约维持保证金率怎么看？防强平爆仓线计算",
  ...
  },
  {
    ...
    tabLabel: "止盈止损设置",
    topicBody: "怎么设止盈止损？双向止盈止损与移动止损实操",
    ...
    heroTitle: "欧易OKX 止盈止损订单配置与锁利润避险指南",
  ...
  },
  {
    ...
    tabLabel: "无限网格",
    topicBody: "无限网格策略怎么玩？牛市单边上涨永不出局设置",
    ...
    heroTitle: "欧意OKX 无限网格机器人配置与牛市滚雪球策略",
    ...
    detailedSteps: [
      ...
      { step: 3, title: "划转资金一键启动【AI策略】", desc: "如果您是新手，可以直接选用欧意OKX AI 智能推荐参数，系统会自动根据历史回测下发最优配置。" }
    ],
    targetedFaq: [
  ...
  },
  {
    ...
    tabLabel: "Passkey免密",
    topicBody: "Passkey免密登录怎么开启？苹果/安卓指纹FaceID绑定",
    ...
    heroTitle: "欧易OKX Passkey 通行密钥设置与生物识别登录指引",
    ...
    detailedSteps: [
      { step: 1, title: "进入 App【安全中心】设置", desc: "登录欧易OKX App，点击左上角个人头像 -> 选择“设置” -> 进入“安全中心” -> 点击“通行密钥 (Passkey)”。" },
  ...
  },
  {
    ...
    tabLabel: "助记词防盗",
    topicBody: "Web3钱包助记词怎么安全备份？防截图与云备份防盗",
    ...
    heroTitle: "欧意OKX Web3 钱包助记词备份与绝对防盗管理守则",
    ...
    targetedFaq: [
      { q: "如果我不小心把助记词丢失了，欧意OKX安全客服能帮我找回钱包里的资产吗？", a: "绝对不能。因为欧意OKX Web3 钱包是非托管去中心化钱包，安全服务器绝对不保存任何用户的助记词。请务必自己做好备份。" },
  ...
  },
  {
    ...
    tabLabel: "DApp授权撤销",
    topicBody: "Web3钱包DApp授权撤销怎么做？防无限许可盗币",
    ...
    heroTitle: "欧易OKX Web3 钱包 DApp 授权管理与取消无限许可实操",
    ...
    detailedSteps: [
      { step: 1, title: "进入 Web3 钱包【授权管理】页面", desc: "打开欧易OKX Web3 钱包 -> 点击“发现/工具” -> 选择“安全检测 / 授权管理”。" },
  ...
  },
  {
    ...
    tabLabel: "C2C申诉流程",
    topicBody: "C2C买币卖币纠纷怎么申诉？客服介入与凭证举证",
    ...
    heroTitle: "欧意OKX C2C 交易订单纠纷处理与客服介入申诉指引",
    ...
    targetedFaq: [
      ...
      { q: "申诉期间，订单里的代币会被黑心商家转移走吗？", a: "绝对不可能。一旦进入申诉状态，托管在欧意OKX系统的代币处于**完全锁定状态**，任何一方都无法私自撤回或转移。" }
    ]
  },
  {
    slug: "okx-fast-convert-zero-fee-guide",
    tabLabel: "快捷闪兑",
    topicBody: "快捷闪兑怎么用？零手续费一键代币转换教程",
    ...
    heroTitle: "欧易OKX 闪兑 (Convert) 零手续费一键换币实操指南",
    ...
    detailedSteps: [
      { step: 1, title: "在交易大厅选择【闪兑 / Convert】", desc: "打开欧易OKX App，在底部交易菜单中点击顶部的模式选择，切换至“闪兑”。" },
  ...
  },
  {
    ...
    tabLabel: "海鸥理财",
    topicBody: "海鸥理财安全吗？看涨与看跌结构化理财收益率结算",
    ...
    heroTitle: "欧意OKX 海鸥 (Seagull) 理财申购逻辑与收益结算指南",
  ...
  },
  {
    ...
    tabLabel: "子账户划转",
    topicBody: "子账户怎么创建？子母账户秒级划转与独立策略管理",
    ...
    heroTitle: "欧易OKX 子账户创建配置与子母账户资产即时划转指南",
    ...
    targetedFaq: [
      { q: "创建欧易OKX子账户需要额外的身份认证 (KYC) 吗？", a: "完全不需要！所有子账户自动继承母账户的 KYC 认证等级，创建后可直接使用，无需重复上传证件。" },
      { q: "子账户里的资金可以随意直接提现到外部区块链钱包吗？", a: "出于安全保护，母账户可开启“禁止子账户提现”开关。开启后子账户资金只能划转回母账户，完全不用担心子账户密码泄露导致的提币损失。" }
    ]
  }
];

// Clean function to trim scContent to base 41 items correctly
let scContent = fs.readFileSync(seoDataPath, 'utf8').replace(/\r\n/g, '\n');
const targetStr = '"publishDate": "2026-07-19"';
const cardApplyDatePos = scContent.indexOf(targetStr);

if (cardApplyDatePos === -1) {
  console.error("❌ Could not find target publishDate for okx-card-apply in seoData.ts");
  process.exit(1);
}

const closeBracePos = scContent.indexOf('}', cardApplyDatePos);
let scBase = scContent.slice(0, closeBracePos + 1);

// Generate 120 full entries using intent suffix rotation
const allGeneratedArticles = [];

for (let i = 0; i < 120; i++) {
  const t = topics[i % topics.length];
  const dateStr = getDateString(startDate, i);
  const cycleIndex = Math.floor(i / topics.length);
  const uniqueSlug = cycleIndex === 0 ? t.slug : `${t.slug}-v${cycleIndex + 1}`;
  
  // Pick alternating front brand: 欧易OKX or 欧意OKX
  const frontBrand = i % 2 === 0 ? "欧易OKX" : "欧意OKX";
  
  // Pick intent suffix from pool
  const scSuffix = scSuffixes[i % scSuffixes.length];
  
  const title = cycleIndex === 0 
    ? `${frontBrand}${t.topicBody}${scSuffix}`
    : `${frontBrand}${t.topicBody} (进阶教程${cycleIndex + 1})${scSuffix}`;

  const article = {
    route: uniqueSlug,
    tabLabel: cycleIndex === 0 ? t.tabLabel : `${t.tabLabel} ${cycleIndex + 1}`,
    title: title,
    description: t.description,
    keywords: t.keywords,
    heroBadge: t.heroBadge,
    heroTitle: `${frontBrand} ${t.heroTitle.replace(/欧[意易]OKX/g, '')}`,
    heroSub: t.heroSub,
...
console.log(`Generated ${allGeneratedArticles.length} clean articles with intent suffixes (下载/网址/网站/电脑版等) spanning from ${allGeneratedArticles[0].publishDate} to ${allGeneratedArticles[allGeneratedArticles.length - 1].publishDate}.`);

// Append to Simplified Chinese
let scAppend = '';
for (const art of allGeneratedArticles) {
  scAppend += `,\n  "${art.route}": ${JSON.stringify(art, null, 2)}`;
}

fs.writeFileSync(seoDataPath, scBase + scAppend + '\n};', 'utf8');
console.log('✅ seoData.ts successfully rebuilt with intent suffixes!');
...
// Re-read seoData.hant.ts original up to base 41 items
let hantContent = fs.readFileSync(seoDataHantPath, 'utf8').replace(/\r\n/g, '\n');
const hantCardApplyDatePos = hantContent.indexOf(targetStr);

if (hantCardApplyDatePos === -1) {
  console.error("❌ Could not find target publishDate for okx-card-apply in seoData.hant.ts");
  process.exit(1);
}

const hantCloseBracePos = hantContent.indexOf('}', hantCardApplyDatePos);

let hantBase = hantContent.slice(0, hantCloseBracePos + 1);

let hantAppend = '';
for (let i = 0; i < allGeneratedArticles.length; i++) {
  const art = allGeneratedArticles[i];
  const hantArt = JSON.parse(JSON.stringify(art));
  
  const frontHantBrand = i % 2 === 0 ? "歐易OKX" : "歐意OKX";
  const hantSuffix = hantSuffixes[i % hantSuffixes.length];
  const cycleIndex = Math.floor(i / topics.length);
  const t = topics[i % topics.length];
  
  hantArt.tabLabel = convertToHant(hantArt.tabLabel);
  hantArt.title = cycleIndex === 0 
    ? `${frontHantBrand}${convertToHant(t.topicBody)}${hantSuffix}`
    : `${frontHantBrand}${convertToHant(t.topicBody)} (進階教程${cycleIndex + 1})${hantSuffix}`;
...
fs.writeFileSync(seoDataHantPath, hantBase + hantAppend + '\n};', 'utf8');
console.log('✅ seoData.hant.ts successfully rebuilt with intent suffixes!');
